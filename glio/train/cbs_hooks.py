...
from . import hooks_base
from ..torch_tools import is_container

class Log_SignalDistribution(hooks_base.LearnerForwardHook):
...
class Log_SignalHistorgram(hooks_base.LearnerForwardHook):
...
class Log_GradDistribution(hooks_base.LearnerBackwardHook):
    def __init__(self, step: int = 1, mean = True, std = True, var = True, min = True, max = True, filt = lambda x: not is_container(x)): # pylint: disable=W0622
        self.step = step
...
class Log_GradHistorgram(hooks_base.LearnerBackwardHook):
